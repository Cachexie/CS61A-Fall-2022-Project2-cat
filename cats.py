...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def f(paragraphs):
        for i in split(lower(remove_punctuation(paragraphs))):
            if i in topic:
                    return True
        return False
    return f
...
def accuracy(typed, source):
    ...
    sl=len(split(source))
    c=0
    for i in range(min(tl,sl)):
        if split(typed)[i]==split(source)[i]:
            c+=1
    if tl==0:
        if sl==0:
            return 100.0
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in word_list:
        if typed_word==i:
            return typed_word
        j=min(j,diff_function(typed_word,i,limit))
    for i in word_list:
        if j==diff_function(typed_word,i,limit):
            return i
    return typed_word
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    if limit<0:
        return 1
    return feline_fixes(typed[1:],source[1:],limit-1)+1 if typed[0]!=source[0] else feline_fixes(typed[1:],source[1:],limit)
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit<0:  # Fill in the condition
        ...
        # BEGIN
        "*** YOUR CODE HERE ***"
        return minimum_mewtations(start[1:],goal[1:],limit) if start[0]==goal[0] else min(add,remove,substitute)+1
        # END
...
def time_per_word(words, times_per_player):
    ...
    for j in range(len(times_per_player)):
        timelist+=[[times_per_player[j][i+1]-times_per_player[j][i] for i in range(len(times_per_player[0])-1)]]
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    gat=get_all_times(match)
    fastest=[[] for _ in player_indices]
    time_per_word=[[gat[i][j] for i in player_indices] for j in word_indices]
    for i in time_per_word:
        j=i.index(min(i))
        fastest[j]+=[get_all_words(match)[time_per_word.index(i)]]
        i.clear()
    return fastest
    # END PROBLEM 10
